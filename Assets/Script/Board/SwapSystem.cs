...
namespace CandyProject
{
    public class SwapSystem
    {
        ...
        private void SwapPiecesDeadlock(int column, int row, Vector2 Direction)
        {
            ...
            if (board.crates[column, row] != null || board.crates[targetPos.x, targetPos.y] != null) return;
        ...
        public bool IsDeadlock()
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    if (board.gems[x, y] == null || board.crates[x, y] != null || board.blankSpaces[x,y])
                        continue;
        ...
        public List<Gem> ListMatches()
        {
            ...
            for (int x = 0; x < width; x++)
            {
                for (int y = 0; y < height; y++)
                {
                    ...
                    if (board.gems[x, y] != null && board.crates[x,y] == null)
                    {
                        if (x < width - 1)
                        {
                            if (SwitchAndCheck(x, y, Vector2.right))
