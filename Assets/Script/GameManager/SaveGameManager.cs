...
namespace CandyProject
{
    ...
    public class ProgressData
    {
        ...
        public ProgressData(List<LevelProgress> progressList)
        {
            this.progressList = new List<LevelProgress>();

            foreach (LevelProgress progress in progressList)
            {
                this.progressList.Add(new LevelProgress(progress));
            }
        }
    }
    ...
    public class ResourceData
    {
        ...
        {
            if (other.boosters != null)
            {
                this.boosters = new Dictionary<GemData, int>();

                foreach (var kvp in other.boosters)
                {
                    this.boosters.Add(kvp.Key, kvp.Value);
                }
            }
            else
            {
    ...
    public static class SaveSystem
    {
        ...
        public static void SaveProgress(List<LevelProgress> levelProgressList)
        {
            try
            {
                ProgressData saveData = new ProgressData(levelProgressList);
        ...
        public static void SaveResource(ResourceData resourceData)
        {
            try
            {
                ResourceData resource = new ResourceData(resourceData);

                string json = JsonUtility.ToJson(resource);
                string encrypted = Encrypt(json, SecretKey);
                File.WriteAllText(ResourcePath, encrypted);
                Debug.Log($"Game Resource saved at: {ResourcePath}");
            }
            catch (Exception e)
            {
